...
suite('EditorUtils', () => {
    ...
    setup(() => {
        ...
        mockDocument = {
            ...
            getWordRangeAtPosition: (position, regexp) => undefined
        };
    });
    suite('getEffectiveRange', () => {
        test('should return selected text when available', () => {
            ...
            const mockDocWithText = {
                ...mockDocument,
                getText: (range) => selectedText
            };
            const result = EditorUtils.getEffectiveRange(mockDocWithText, mockRange);
        ...
        test('should return null for empty line', () => {
            ...
            const result = EditorUtils.getEffectiveRange(mockDocWithEmptyLine, mockRange);
        ...
        test('should expand empty selection to full lines', () => {
            ...
            const result = EditorUtils.getEffectiveRange(mockDocWithLines, initialRange);
            assert.ok(result, 'Result should not be null');
    ...
    suite('hasIntersectingRange', () => {
        test('should return false for ranges that only touch boundaries', () => {
            ...
            assert.strictEqual(EditorUtils.hasIntersectingRange(range1, range2), false, 'Ranges that only touch should not intersect');
        });
        test('should return true for overlapping ranges', () => {
            ...
            const range2 = new vscode.Range(new vscode.Position(0, 10), new vscode.Position(0, 20));
            assert.strictEqual(EditorUtils.hasIntersectingRange(range1, range2), true, 'Overlapping ranges should intersect');
        });
        test('should return false for non-overlapping ranges', () => {
            ...
            assert.strictEqual(EditorUtils.hasIntersectingRange(range1, range2), false, 'Non-overlapping ranges should not intersect');
        });
    });
    suite('getFilePath', () => {
        test('should return relative path when in workspace', () => {
            ...
            vscode.workspace.getWorkspaceFolder = (uri) => mockWorkspaceFolder;
            try {
...
//# sourceMappingURL=EditorUtils.test.js.map