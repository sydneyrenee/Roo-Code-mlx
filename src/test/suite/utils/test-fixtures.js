 ...
 */
export function createTestDocument(content, language = 'typescript') {
    const lines = content.split('\n');
    function createTextLine(lineNumber) {
        const text = lines[lineNumber] || '';
        const range = new vscode.Range(new vscode.Position(lineNumber, 0), new vscode.Position(lineNumber, text.length));
        const rangeIncludingLineBreak = new vscode.Range(new vscode.Position(lineNumber, 0), new vscode.Position(lineNumber, text.length + 1));
 ...
 */
export function createTestWorkspaceFolders() {
    return [{
 ...
 */
export function createTestConfiguration(custom = false) {
    return custom ? TEST_FIXTURES.CONFIG.CUSTOM : TEST_FIXTURES.CONFIG.DEFAULT;
}
/**
 * Create test state
 */
export function createTestState(scope = 'global') {
