...
suite("Roo Code Modes", () => {
    test("Should handle switching modes correctly", async function () {
        const timeout = 30000;
        const interval = 1000;
        const testPrompt = "For each mode (Code, Architect, Ask) respond with the mode name and what it specializes in after switching to that mode, do not start with the current mode, be sure to say 'I AM DONE' after the task is complete";
        ...
        try {
            ...
            while (Date.now() - startTime < timeout) {
                const messages = globalThis.provider.messages;
                if (messages.some(({ type, text }) => type === "say" && text?.includes("I AM DONE GRADING") && !text?.includes("be sure to say"))) {
                    break;
                }
                await new Promise((resolve) => setTimeout(resolve, interval));
            }
            ...
            const gradeMessage = globalThis.provider.messages.find(({ type, text }) => type === "say" && !text?.includes("Grade: (1-10)") && text?.includes("Grade:"))?.text;
