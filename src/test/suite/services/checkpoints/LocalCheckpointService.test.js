...
suite('LocalCheckpointService', () => {
    ...
    suite('saveCheckpoint', () => {
        ...
        test('throws if we\'re on the wrong branch', async () => {
            ...
            await assert.rejects(() => service.saveCheckpoint('test'), new RegExp(`Git branch mismatch: expected '${currentBranch}' but found 'feature'`), 'Should throw on wrong branch');
            // Attempt to restore checkpoint from feature branch
            assert.ok(service.baseHash, 'Base hash should exist');
            await assert.rejects(() => service.restoreCheckpoint(service.baseHash), new RegExp(`Git branch mismatch: expected '${currentBranch}' but found 'feature'`), 'Should throw on wrong branch');
        });
        test('cleans up staged files if a commit fails', async () => {
            ...
            try {
                await assert.rejects(() => service.saveCheckpoint('test'), /Simulated commit failure/, 'Should throw commit error');
    ...
    suite('create', () => {
        test('initializes a git repository if one does not already exist', async () => {
            ...
            assert.ok(commit1?.commit, 'Should create first commit');
            assert.strictEqual(await fs.readFile(newTestFile, 'utf-8'), 'Hello, world!', 'File content should match');
            ...
            await assert.rejects(() => fs.access(newTestFile), 'File should not exist');
            // Restore to checkpoint 1; the file should now exist
            await newService.restoreCheckpoint(commit1.commit);
            assert.strictEqual(await fs.readFile(newTestFile, 'utf-8'), 'Hello, world!', 'File should be restored');
            ...
            // Restore "Hello, world!"
            await newService.restoreCheckpoint(commit1.commit);
            assert.strictEqual(await fs.readFile(newTestFile, 'utf-8'), 'Hello, world!', 'Should restore first checkpoint');
            // Restore "Ahoy, world!"
            await newService.restoreCheckpoint(commit2.commit);
            assert.strictEqual(await fs.readFile(newTestFile, 'utf-8'), 'Ahoy, world!', 'Should restore second checkpoint');
...
//# sourceMappingURL=LocalCheckpointService.test.js.map