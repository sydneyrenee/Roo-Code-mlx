...
async function loadRuleFiles(cwd) {
    ...
    for (const file of ruleFiles) {
        try {
            const content = await promises_1.default.readFile(path_1.default.join(cwd, file), "utf-8");
            if (content.trim()) {
                combinedRules += `\n# Rules from ${file}:\n${content.trim()}\n`;
            }
        }
...
async function addCustomInstructions(modeCustomInstructions, globalCustomInstructions, cwd, mode, options = {}) {
    ...
    if (mode) {
        try {
            ...
            const content = await promises_1.default.readFile(path_1.default.join(cwd, modeRuleFile), "utf-8");
            if (content.trim()) {
