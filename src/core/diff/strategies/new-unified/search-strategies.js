...
exports.prepareSearchString = prepareSearchString;
exports.evaluateSimilarity = evaluateSimilarity;
exports.getDMPSimilarity = getDMPSimilarity;
exports.validateEditResult = validateEditResult;
exports.findExactMatch = findExactMatch;
exports.findSimilarityMatch = findSimilarityMatch;
exports.findLevenshteinMatch = findLevenshteinMatch;
exports.findAnchorMatch = findAnchorMatch;
...
function prepareSearchString(changes) {
...
function evaluateSimilarity(original, modified) {
    return (0, string_similarity_1.compareTwoStrings)(original, modified);
}
// Helper function to validate using diff-match-patch
function getDMPSimilarity(original, modified) {
...
function validateEditResult(hunk, result) {
...
function findExactMatch(searchStr, content, startIndex = 0, confidenceThreshold = 0.97) {
    const searchLines = searchStr.split("\n");
...
function findSimilarityMatch(searchStr, content, startIndex = 0, confidenceThreshold = 0.97) {
    const searchLines = searchStr.split("\n");
    let bestScore = 0;
    let bestIndex = -1;
    for (let i = startIndex; i < content.length - searchLines.length + 1; i++) {
        const windowStr = content.slice(i, i + searchLines.length).join("\n");
        const score = (0, string_similarity_1.compareTwoStrings)(searchStr, windowStr);
...
function findLevenshteinMatch(searchStr, content, startIndex = 0, confidenceThreshold = 0.97) {
    const searchLines = searchStr.split("\n");
    const candidates = [];
    for (let i = startIndex; i < content.length - searchLines.length + 1; i++) {
    ...
    if (candidates.length > 0) {
        const closestMatch = (0, fastest_levenshtein_1.closest)(searchStr, candidates);
...
function findAnchorMatch(searchStr, content, startIndex = 0, confidenceThreshold = 0.97) {
    const searchLines = searchStr.split("\n");
...
function findBestMatch(searchStr, content, startIndex = 0, confidenceThreshold = 0.97) {
