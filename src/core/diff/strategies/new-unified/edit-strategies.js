...
exports.applyContextMatching = applyContextMatching;
exports.applyDMP = applyDMP;
exports.applyGitFallback = applyGitFallback;
...
function applyContextMatching(hunk, content, matchPosition) {
    if (matchPosition === -1) {
    ...
    const confidence = (0, search_strategies_1.validateEditResult)(hunk, afterText);
    return {
        confidence,
        result: newResult,
        strategy: "context",
...
// DMP edit strategy
function applyDMP(hunk, content, matchPosition) {
    if (matchPosition === -1) {
    ...
    const confidence = (0, search_strategies_1.validateEditResult)(hunk, patchedText);
    return {
        confidence,
        result: newResult,
        strategy: "dmp",
...
async function applyGitFallback(hunk, content) {
    let tmpDir;
    try {
        tmpDir = tmp.dirSync({ unsafeCleanup: true });
        const git = (0, simple_git_1.default)(tmpDir.name);
        await git.init();
...
async function applyEdit(hunk, content, matchPosition, confidence, confidenceThreshold = 0.97) {
