...
exports.NewUnifiedDiffStrategy = void 0;
const search_strategies_1 = require("./search-strategies");
const edit_strategies_1 = require("./edit-strategies");
class NewUnifiedDiffStrategy {
    confidenceThreshold;
...
Your diff here
    ...
    async applyDiff(originalContent, diffContent, startLine, endLine) {
        ...
        for (const hunk of parsedDiff.hunks) {
            const contextStr = (0, search_strategies_1.prepareSearchString)(hunk.changes);
            const { index: matchPosition, confidence, strategy, } = (0, search_strategies_1.findBestMatch)(contextStr, result, 0, this.confidenceThreshold);
            if (confidence < this.confidenceThreshold) {
                ...
                for (const subHunk of subHunks) {
                    const subContextStr = (0, search_strategies_1.prepareSearchString)(subHunk.changes);
                    const subSearchResult = (0, search_strategies_1.findBestMatch)(subContextStr, subHunkResult, 0, this.confidenceThreshold);
                    if (subSearchResult.confidence >= this.confidenceThreshold) {
                        const subEditResult = await (0, edit_strategies_1.applyEdit)(subHunk, subHunkResult, subSearchResult.index, subSearchResult.confidence, this.confidenceThreshold);
            ...
            const editResult = await (0, edit_strategies_1.applyEdit)(hunk, result, matchPosition, confidence, this.confidenceThreshold);
...
exports.NewUnifiedDiffStrategy = NewUnifiedDiffStrategy;
