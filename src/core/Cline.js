...
const mode_validator_1 = require("./mode-validator");
...
const TerminalManager_1 = require("../integrations/terminal/TerminalManager");
...
const cwd = vscode.workspace.workspaceFolders?.map((folder) => folder.uri.fsPath).at(0) ?? path.join(os_1.default.homedir(), "Desktop"); // may or may not exist but fs checking existence would immediately ask for permission which would be bad UX, need to come up with a better solution
class Cline {
    taskId;
    api;
    terminalManager;
    ...
    constructor(provider, apiConfiguration, customInstructions, enableDiff, enableCheckpoints, fuzzyMatchThreshold, task, images, historyItem, experiments) {
        ...
        this.taskId = crypto_1.default.randomUUID();
        ...
        this.diffViewProvider = new DiffViewProvider_1.DiffViewProvider(cwd);
        this.checkpointsEnabled = enableCheckpoints ?? false;
        if (historyItem) {
            this.taskId = historyItem.id;
        }
        // Initialize diffStrategy based on current state
        this.updateDiffStrategy(experiments_1.experiments.isEnabled(experiments ?? {}, experiments_1.EXPERIMENT_IDS.DIFF_STRATEGY));
        if (task || images) {
    ...
    async updateDiffStrategy(experimentalDiffStrategy) {
        ...
        if (experimentalDiffStrategy === undefined) {
            ...
            experimentalDiffStrategy = stateExperimental?.[experiments_1.EXPERIMENT_IDS.DIFF_STRATEGY] ?? false;
        }
        this.diffStrategy = (0, DiffStrategy_1.getDiffStrategy)(this.api.getModel().id, this.fuzzyMatchThreshold, experimentalDiffStrategy);
    }
    ...
    async ensureTaskDirectoryExists() {
        ...
        await promises_1.default.mkdir(taskDir, { recursive: true });
        return taskDir;
    }
    async getSavedApiConversationHistory() {
        const filePath = path.join(await this.ensureTaskDirectoryExists(), ClineProvider_1.GlobalFileNames.apiConversationHistory);
    ...
    async saveApiConversationHistory() {
        try {
            const filePath = path.join(await this.ensureTaskDirectoryExists(), ClineProvider_1.GlobalFileNames.apiConversationHistory);
            await promises_1.default.writeFile(filePath, JSON.stringify(this.apiConversationHistory));
        }
        catch (error) {
    ...
    async getSavedClineMessages() {
        const filePath = path.join(await this.ensureTaskDirectoryExists(), ClineProvider_1.GlobalFileNames.uiMessages);
        if (await (0, fs_1.fileExistsAtPath)(filePath)) {
            ...
            if (await (0, fs_1.fileExistsAtPath)(oldPath)) {
                const data = JSON.parse(await promises_1.default.readFile(oldPath, "utf8"));
                await promises_1.default.unlink(oldPath); // remove old file
                return data;
            }
        }
        return [];
    ...
    async saveClineMessages() {
        try {
            ...
            const filePath = path.join(taskDir, ClineProvider_1.GlobalFileNames.uiMessages);
            await promises_1.default.writeFile(filePath, JSON.stringify(this.clineMessages));
            // combined as they are in ChatView
            const apiMetrics = (0, getApiMetrics_1.getApiMetrics)((0, combineApiRequests_1.combineApiRequests)((0, combineCommandSequences_1.combineCommandSequences)(this.clineMessages.slice(1))));
            const taskMessage = this.clineMessages[0]; // first message is always the task say
            const lastRelevantMessage = this.clineMessages[(0, array_1.findLastIndex)(this.clineMessages, (m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"))];
            let taskDirSize = 0;
            try {
                taskDirSize = await get_folder_size_1.default.loose(taskDir);
            }
            catch (err) {
                console.error(`[saveClineMessages] failed to get task directory size (${taskDir}): ${err instanceof Error ? err.message : String(err)}`);
            }
    ...
    async say(type, text, images, partial, checkpoint) {
        if (this.abort) {
            throw new Error("Roo Code instance aborted");
        }
        if (partial !== undefined) {
    ...
    async executeCommandTool(command) {
        ...
        const output = (0, extract_text_1.truncateOutput)(lines.join("\n"), terminalOutputLineLimit);
        const result = output.trim();
        if (userFeedback) {
            await this.say("user_feedback", userFeedback.text, userFeedback.images);
            return [
                true,
                responses_1.formatResponse.toolResult(`Command is still running in the user's terminal.${result.length > 0 ? `\nHere's the output so far:\n${result}` : ""}\n\nThe user provided the following feedback:\n<feedback>\n${userFeedback.text}\n</feedback>`, userFeedback.images),
            ];
        }
        if (completed) {
    ...
    async *attemptApiRequest(previousApiReqIndex, retryAttempt = 0) {
        let mcpHub;
        const { mcpEnabled, alwaysApproveResubmit, requestDelaySeconds, rateLimitSeconds } = (await this.providerRef.deref()?.getState()) ?? {};
        ...
        if (mcpEnabled ?? true) {
            ...
            await (0, p_wait_for_1.default)(() => mcpHub.isConnecting !== true, { timeout: 10_000 }).catch(() => {
                console.error("MCP servers failed to connect in time");
            });
        }
        const { browserViewportSize, mode, customModePrompts, preferredLanguage, experiments, enableMcpServerCreation, } = (await this.providerRef.deref()?.getState()) ?? {};
        ...
        const systemPrompt = await (async () => {
            ...
            return (0, system_1.SYSTEM_PROMPT)(provider.context, cwd, this.api.getModel().info.supportsComputerUse ?? false, mcpHub, this.diffStrategy, browserViewportSize, mode, customModePrompts, customModes, this.customInstructions, preferredLanguage, this.diffEnabled, experiments, enableMcpServerCreation);
        })();
        ...
        if (previousApiReqIndex >= 0) {
            ...
            if (!previousRequest)
                return;
            const { tokensIn = 0, tokensOut = 0, cacheWrites = 0, cacheReads = 0, } = JSON.parse(previousRequest);
            const totalTokens = tokensIn + tokensOut + cacheWrites + cacheReads;
            const trimmedMessages = (0, sliding_window_1.truncateConversationIfNeeded)(this.apiConversationHistory, totalTokens, this.api.getModel().info);
    ...
    async presentAssistantMessage() {
        ...
        const block = (0, clone_deep_1.default)(this.assistantMessageContent[this.currentStreamingContentIndex]); // need to create copy bc while stream is updating the array, it could be updating the reference block properties too
            ...
            case "tool_use":
                ...
                const askApproval = async (type, partialMessage) => {
                    ...
                    if (response !== "yesButtonClicked") {
                        ...
                        if (text) {
                            await this.say("user_feedback", text, images);
                            pushToolResult(responses_1.formatResponse.toolResult(responses_1.formatResponse.toolDeniedWithFeedback(text), images));
                        }
                        else {
                    ...
                    if (text) {
                        await this.say("user_feedback", text, images);
                        pushToolResult(responses_1.formatResponse.toolResult(responses_1.formatResponse.toolApprovedWithFeedback(text), images));
                    }
                    return true;
                };
                ...
                try {
                    (0, mode_validator_1.validateToolUse)(block.name, mode ?? modes_1.defaultModeSlug, customModes ?? [], {
                        apply_diff: this.diffEnabled,
                    }, block.params);
                }
                catch (error) {
                    this.consecutiveMistakeCount++;
                    pushToolResult(responses_1.formatResponse.toolError(error.message));
                    break;
                }
                switch (block.name) {
                    case "write_to_file": {
                        ...
                        const sharedMessageProps = {
                            ...
                            path: (0, path_1.getReadablePath)(cwd, removeClosingTag("path", relPath)),
                        };
                        try {
                            if (block.partial) {
                                // update gui message
                                ...
                                await this.diffViewProvider.update((0, extract_text_1.everyLineHasLineNumbers)(newContent) ? (0, extract_text_1.stripLineNumbers)(newContent) : newContent, false);
                                break;
                            }
                            else {
                                if (!relPath) {
                                ...
                                await this.diffViewProvider.update((0, extract_text_1.everyLineHasLineNumbers)(newContent) ? (0, extract_text_1.stripLineNumbers)(newContent) : newContent, true);
                                await (0, delay_1.default)(300); // wait for diff view to update
                                this.diffViewProvider.scrollToFirstDiff();
                                // Check for code omissions before proceeding
                                if ((0, detect_omission_1.detectCodeOmission)(this.diffViewProvider.originalContent || "", newContent, predictedLineCount)) {
                                ...
                                const completeMessage = JSON.stringify({
                                    ...
                                    diff: fileExists
                                        ? responses_1.formatResponse.createPrettyPatch(relPath, this.diffViewProvider.originalContent, newContent)
                                        : undefined,
                                });
                                ...
                                if (userEdits) {
                                    await this.say("user_feedback_diff", JSON.stringify({
                                        tool: fileExists ? "editedExistingFile" : "newFileCreated",
                                        path: (0, path_1.getReadablePath)(cwd, relPath),
                                        diff: userEdits,
                                    }));
                    ...
                    case "apply_diff": {
                        ...
                        try {
                            if (block.partial) {
                                ...
                                const diffResult = (await this.diffStrategy?.applyDiff(originalContent, diffContent, parseInt(block.params.start_line ?? ""), parseInt(block.params.end_line ?? ""))) ?? {
                                    success: false,
                    ...
                    case "insert_content": {
                        const relPath = block.params.path;
                        const operations = block.params.operations;
                        const sharedMessageProps = {
                            tool: "appliedDiff",
                            path: (0, path_1.getReadablePath)(cwd, removeClosingTag("path", relPath)),
                        };
                        try {
                            ...
                            try {
                                ...
                                pushToolResult(responses_1.formatResponse.toolError("Invalid operations JSON format"));
                                break;
                            }
                            this.consecutiveMistakeCount = 0;
                            // Read the file
                            const fileContent = await promises_1.default.readFile(absolutePath, "utf8");
                            this.diffViewProvider.editType = "modify";
                            this.diffViewProvider.originalContent = fileContent;
                            const lines = fileContent.split("\n");
                            const updatedContent = (0, insert_groups_1.insertGroups)(lines, parsedOperations.map((elem) => {
                                return {
                            ...
                            const diff = responses_1.formatResponse.createPrettyPatch(relPath, fileContent, updatedContent);
                            if (!diff) {
                            ...
                            const didApprove = await this.ask("tool", completeMessage, false).then((response) => response.response === "yesButtonClicked");
                            if (!didApprove) {
                                await this.diffViewProvider.revertChanges();
                    ...
                    case "read_file": {
                        ...
                        try {
                            if (block.partial) {
                                ...
                                const content = await (0, extract_text_1.extractTextFromFile)(absolutePath);
                    ...
                    case "list_files": {
                        ...
                        const sharedMessageProps = {
                            ...
                            path: (0, path_1.getReadablePath)(cwd, removeClosingTag("path", relDirPath)),
                        };
                        try {
                            if (block.partial) {
                                const partialMessage = JSON.stringify({
                                ...
                                const result = responses_1.formatResponse.formatFilesList(absolutePath, files, didHitLimit);
                                const completeMessage = JSON.stringify({
                                    ...sharedMessageProps,
                                    content: result,
                                });
                    ...
                    case "list_code_definition_names": {
                        ...
                        try {
                            if (block.partial) {
                                ...
                                this.consecutiveMistakeCount = 0;
                                const absolutePath = path.resolve(cwd, relDirPath);
                                const result = await (0, tree_sitter_1.parseSourceCodeForDefinitionsTopLevel)(absolutePath);
                                const completeMessage = JSON.stringify({
                                    ...sharedMessageProps,
                                    content: result,
                                });
                    ...
                    case "search_files": {
                        ...
                        try {
                            if (block.partial) {
                                ...
                                this.consecutiveMistakeCount = 0;
                                const absolutePath = path.resolve(cwd, relDirPath);
                                const results = await (0, ripgrep_1.regexSearchFiles)(cwd, absolutePath, regex, filePattern);
                                const completeMessage = JSON.stringify({
                                    ...sharedMessageProps,
                                    content: results,
                                });
                            ...
                            if (block.partial) {
                                ...
                                if (mcp_arguments) {
                                    try {
                                        ...
                                        this.consecutiveMistakeCount++;
                                        await this.say("error", `Roo tried to use ${tool_name} with an invalid JSON argument. Retrying...`);
                                        pushToolResult(responses_1.formatResponse.toolError(responses_1.formatResponse.invalidMcpToolArgumentError(server_name, tool_name)));
                                        break;
                                    }
                                }
                                this.consecutiveMistakeCount = 0;
                                ...
                                pushToolResult(responses_1.formatResponse.toolResult(toolResultPretty));
                                break;
                            }
                        }
                        catch (error) {
                    ...
                    case "access_mcp_resource": {
                        ...
                        try {
                            if (block.partial) {
                                ...
                                pushToolResult(responses_1.formatResponse.toolResult(resourceResultPretty));
                                break;
                            }
                        }
                        catch (error) {
                    ...
                    case "switch_mode": {
                        ...
                        try {
                            if (block.partial) {
                                ...
                                const targetMode = (0, modes_1.getModeBySlug)(mode_slug, (await this.providerRef.deref()?.getState())?.customModes);
                                if (!targetMode) {
                                    pushToolResult(responses_1.formatResponse.toolError(`Invalid mode: ${mode_slug}`));
                                    break;
                                }
                                // Check if already in requested mode
                                const currentMode = (await this.providerRef.deref()?.getState())?.mode ?? modes_1.defaultModeSlug;
                    ...
                    case "new_task": {
                        ...
                        try {
                            if (block.partial) {
                                ...
                                this.consecutiveMistakeCount = 0;
                                // Verify the mode exists
                                const targetMode = (0, modes_1.getModeBySlug)(mode, (await this.providerRef.deref()?.getState())?.customModes);
                                if (!targetMode) {
                                ...
                                if (provider) {
                                    ...
                                    pushToolResult(responses_1.formatResponse.toolError("Failed to create new task: provider not available"));
                                }
                                break;
                            }
                        }
    ...
    async recursivelyMakeClineRequests(userContent, includeFileDetails = false) {
        ...
        if (this.consecutiveMistakeCount >= 3) {
            ...
            if (response === "messageResponse") {
                userContent.push(...[
                    {
                        type: "text",
                        text: responses_1.formatResponse.tooManyMistakes(text),
                    },
        ...
        const previousApiReqIndex = (0, array_1.findLastIndex)(this.clineMessages, (m) => m.say === "api_req_started");
        ...
        await this.say("api_req_started", JSON.stringify({
            request: userContent.map((block) => (0, export_markdown_1.formatContentBlockToMarkdown)(block)).join("\n\n") + "\n\nLoading...",
        }));
        ...
        try {
            ...
            const updateApiReqMsg = (cancelReason, streamingFailedMessage) => {
                this.clineMessages[lastApiReqIndex].text = JSON.stringify({
                    ...
                    cost: totalCost ??
                        (0, cost_1.calculateApiCost)(this.api.getModel().info, inputTokens, outputTokens, cacheWriteTokens, cacheReadTokens),
                    cancelReason,
            ...
            try {
                for await (const chunk of stream) {
                    ...
                    switch (chunk.type) {
                        ...
                        case "text":
                            ...
                            this.assistantMessageContent = (0, assistant_message_1.parseAssistantMessage)(assistantMessage);
                ...
                if (!this.abandoned) {
                    ...
                    await abortStream("streaming_failed", error.message ?? JSON.stringify((0, serialize_error_1.serializeError)(error), null, 2));
            ...
            Promise.all(userContent.map(async (block) => {
                ...
                if (block.type === "text") {
                    if (shouldProcessMentions(block.text)) {
                        return {
                            ...block,
                            text: await (0, mentions_1.parseMentions)(block.text, cwd, this.urlContentFetcher),
                        };
                    }
                    return block;
                }
                    ...
                    if (typeof block.content === "string") {
                        if (shouldProcessMentions(block.content)) {
                            return {
                                ...block,
                                content: await (0, mentions_1.parseMentions)(block.content, cwd, this.urlContentFetcher),
                            };
                        }
                        return block;
                    }
                        ...
                        const parsedContent = await Promise.all(block.content.map(async (contentBlock) => {
                            if (contentBlock.type === "text" && shouldProcessMentions(contentBlock.text)) {
                                return {
                                    ...contentBlock,
                                    text: await (0, mentions_1.parseMentions)(contentBlock.text, cwd, this.urlContentFetcher),
                                };
                            }
                            return contentBlock;
                        }));
    ...
    async getEnvironmentDetails(includeFileDetails = false) {
        ...
        if (busyTerminals.length > 0) {
            ...
            await (0, p_wait_for_1.default)(() => busyTerminals.every((t) => !this.terminalManager.isProcessHot(t.id)), {
                interval: 100,
                timeout: 15_000,
            }).catch(() => { });
        }
        ...
        */
        ...
        const { contextTokens } = (0, getApiMetrics_1.getApiMetrics)(this.clineMessages);
        ...
        if (!(0, mode_validator_1.isToolAllowedForMode)("write_to_file", currentMode, customModes ?? [], {
            apply_diff: this.diffEnabled,
        }) &&
            !(0, mode_validator_1.isToolAllowedForMode)("apply_diff", currentMode, customModes ?? [], { apply_diff: this.diffEnabled })) {
            const currentModeName = (0, modes_1.getModeBySlug)(currentMode, customModes)?.name ?? currentMode;
            const defaultModeName = (0, modes_1.getModeBySlug)(modes_1.defaultModeSlug, customModes)?.name ?? modes_1.defaultModeSlug;
        ...
        if (includeFileDetails) {
            ...
            const isDesktop = (0, path_1.arePathsEqual)(cwd, path.join(os_1.default.homedir(), "Desktop"));
            if (isDesktop) {
                // don't want to immediately access desktop since it would show permission popup
                details += "(Desktop files not shown automatically. Use list_files to explore if needed.)";
            }
            else {
                const [files, didHitLimit] = await (0, list_files_1.listFiles)(cwd, true, 200);
                const result = responses_1.formatResponse.formatFilesList(cwd, files, didHitLimit);
                details += result;
            }
        }
    ...
    async getCheckpointService() {
        ...
        if (!this.checkpointService) {
            ...
            this.checkpointService = await checkpoints_1.CheckpointServiceFactory.create({
                strategy: "shadow",
                options: {
                    taskId: this.taskId,
                    workspaceDir,
    ...
    async checkpointDiff({ ts, commitHash, mode, }) {
        if (!this.checkpointsEnabled) {
            return;
        }
        ...
        try {
            ...
            await vscode.commands.executeCommand("vscode.changes", mode === "full" ? "Changes since task started" : "Changes since previous checkpoint", changes.map((change) => [
    ...
    async checkpointRestore({ ts, commitHash, mode, }) {
        if (!this.checkpointsEnabled) {
            return;
        }
        ...
        try {
            ...
            if (mode === "restore") {
                await this.overwriteApiConversationHistory(this.apiConversationHistory.filter((m) => !m.ts || m.ts < ts));
                const deletedMessages = this.clineMessages.slice(index + 1);
                const { totalTokensIn, totalTokensOut, totalCacheWrites, totalCacheReads, totalCost } = (0, getApiMetrics_1.getApiMetrics)((0, combineApiRequests_1.combineApiRequests)((0, combineCommandSequences_1.combineCommandSequences)(deletedMessages)));
                await this.overwriteClineMessages(this.clineMessages.slice(0, index + 1));
                // TODO: Verify that this is working as expected.
                await this.say("api_req_deleted", JSON.stringify({
