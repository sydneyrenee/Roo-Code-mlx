...
interface ExtendedMessageContent extends Omit<MessageContent, 'output'> {
    id?: string;
    tool_use_id?: string;
    content?: ToolContent[];
...
export function convertToBedrockConverseMessages(anthropicMessages: Anthropic.Messages.MessageParam[]): Message[] {
    return anthropicMessages.map((anthropicMessage) => {
        ...
        const content = anthropicMessage.content.map((block) => {
            const messageBlock = block as ExtendedMessageContent;
                        ...
                        toolResult: {
                            ...
                            content: messageBlock.output.map((part) => {
                                if (typeof part === "object" && part !== null) {
                                    const toolPart = part as { type?: string; text?: string };
                                    if ("text" in toolPart) {
                                        return { text: toolPart.text || "" }
                                    }
                                    if ("type" in toolPart && toolPart.type === "image") {
